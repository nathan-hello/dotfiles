...
local override_formatters = {
  json = function()
    vim.cmd("silent !cd " ..
      vim.fn.expand('%:p:h') .. " && prettier --write " .. vim.fn.shellescape(vim.api.nvim_buf_get_name(0))) -- Call prettier binary inside of the directory that I'm in right now (for repo-specific configuration)
  end,
  astro = function()
    vim.cmd("silent !cd " ..
      vim.fn.expand('%:p:h') .. " && prettier --write " .. vim.fn.shellescape(vim.api.nvim_buf_get_name(0))) -- Call prettier binary inside of the directory that I'm in right now (for repo-specific configuration)
...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(event)
    ...
    map("gd", go_to_definition, "[G]oto [D]efinition")                                                      -- Jump to the definition of the word under your cursor. To jump back, press <C-T>.
