...
/* static size_t write_callback(void *contents, size_t size, size_t nmemb, void *userp) {
    ...
    *data = realloc(*data, (*data ? strlen(*data) + size * nmemb : size * nmemb) + 1);
    if (!*data) { 
        free(*data); 
...
static char* weather() {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://api.open-meteo.com/v1/forecast?latitude=" LAT "&longitude=" LON "&current=temperature_2m");
    ...
    if (res != CURLE_OK) {
        ...
        char *temp_start = strstr(readBuffer, "\"current\":");
        if (temp_start) {
            temp_start += 10;
            temp_start = strstr(readBuffer, "\"temperature_2m\":"); 
            if(temp_start) {
                temp_start += 17; 
                char *temp_end = strchr(temp_start, '}');
                if (temp_end) {
                    *temp_end = '\0';
                    double temp = atof(temp_start);

                    free(readBuffer);
                    curl_easy_cleanup(curl);
                    curl_slist_free_all(headers);
                    RETURN_FORMAT(10, "%.2f°C\n", temp);
                }
            }
        }
        curl_slist_free_all(headers);
